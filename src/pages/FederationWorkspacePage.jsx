    ...
    <div className="min-h-screen bg-slate-100 text-slate-800">
      <div className="flex min-h-screen">
        <aside className="hidden w-72 flex-col bg-slate-950 px-4 py-6 text-slate-200 md:flex">
          ...
          <nav className="space-y-1">{sections.map(({ label, icon: Icon }) => <button key={label} type="button" onClick={() => setActiveSection(label)} className={`flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-medium transition ${activeSection === label ? 'bg-sky-500/20 text-white ring-1 ring-sky-400/30' : 'text-slate-300 hover:bg-slate-800 hover:text-white'}`}><Icon className="h-4 w-4" />{label}</button>)}</nav>
        ...
        <main className="min-w-0 flex-1">
          ...
          <div className="p-5 sm:p-8">
            <div className="mb-6 flex gap-2 overflow-x-auto md:hidden">{sections.map(({ label }) => <button key={label} type="button" onClick={() => setActiveSection(label)} className={`whitespace-nowrap rounded-lg px-3 py-2 text-sm font-medium ${activeSection === label ? 'bg-slate-900 text-white' : 'bg-white text-slate-500'}`}>{label}</button>)}</div>
